...
4. **Triage.** A Liquid LFM2.5 model on the laptop reads the repo and writes a plain-English summary before any paid call.
5. **Research.** A Nimble Web Search Agent finds out why it is trending and returns cited, confidence-graded claims.
6. **Broadcast.** An OpenAI-compatible LLM writes a 12-second script, FLUX 3 renders Scout reading it on camera, and the control room airs it with the sources as on-screen captions.
...
| **Black Forest Labs (FLUX)** | Scout's face and voice | Lip-synced FLUX 3 bulletins, draft and final cut |
| **Liquid AI** | On-device reflexes | One-line summaries on the board, written on the laptop |
...
- **Cited and graded.** Each run returns 14 to 22 claims with source URLs and confidence grades. The best-supported ones become the captions under the anchor, with their source and grade on screen.
...
- **Scout, a consistent anchor.** FLUX.2 [pro] cast Scout, a beagle news hound in a charcoal blazer, once. Every bulletin pins that exact still as frame one of a FLUX 3 image-to-video render, so the same anchor is on air all day. A dog anchor also means nobody mistakes AI-written news for a real journalist.
- **Spoken, lip-synced bulletins.** The prompt quotes the script, directs Scout's delivery and forbids on-screen text, so FLUX 3 speaks the line with synchronized audio instead of burning it in. Our own captions are overlaid in the browser.
- **Draft, then final.** Every video starts as a 12-second draft (72 credits, about two minutes). "Render final cut" promotes a keeper with `draft_enhance`: the same take, re-rendered at 1080p (348 credits).
- **Budget-aware.** Auto-detected stories air as text, video is reserved for stories a producer assigns, and a rolling 24-hour credit cap, tracked in RawTree, stops runaway spend.

## Liquid AI

- **On-device triage.** LFM2.5-1.2B runs on the demo laptop through Ollama's OpenAI-compatible server. It reads each repo's description and README and writes a plain-English one-liner for every repo on the board, before any paid call.
- **Private and free.** Summaries cost nothing per call, take about 2.5 seconds each on the laptop's CPU, and never leave the machine. Each one lands in the newsroom log with its on-device latency.
